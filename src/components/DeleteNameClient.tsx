    ...
    <form onSubmit={onDelete} className="w-full max-w-md text-left">
      ...
      <div className="mt-6 flex items-center gap-5">
        <button
          ...
          className="cursor-pointer border border-[color-mix(in_srgb,var(--ink)_45%,transparent)] px-5 py-2.5 font-display text-[1.15rem] leading-none tracking-[-0.02em] text-[var(--ink)] disabled:cursor-not-allowed disabled:opacity-30"
        >
          {pending ? "…" : "delete forever"}
        </button>
        <a
          href={`/settings/${handle}`}
          className="font-display text-[1.15rem] leading-none tracking-[-0.02em] text-[var(--ink-muted)]"
        >
          cancel
        </a>
      </div>
